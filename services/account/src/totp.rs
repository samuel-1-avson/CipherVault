...
mod tests {
    ...
    async fn totp_enrollment_login_and_replay_protection() {
        ...
        let secret = base32_decode(enrollment_json["secret_base32"].as_str().unwrap()).unwrap();
        let current_step = now_utc() / STEP_SECONDS;
        let code = code_for_step(&secret, current_step.saturating_sub(1)).unwrap();
        let confirmed = app
            .clone()
            .oneshot(
        ...
        let next_code = code_for_step(&secret, current_step).unwrap();
        let verified = app
            .clone()
            .oneshot(
